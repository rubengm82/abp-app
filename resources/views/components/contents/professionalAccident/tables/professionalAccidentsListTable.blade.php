    ...
    <tbody>
        @foreach($accidents as $accident)
            <tr class="hover:bg-base-300 transition-colors text-xs">
                <td class="px-4 py-2">
                    <span class="badge badge-dash whitespace-nowrap text-center min-w-0 max-w-full truncate {{ $accident->type === 'Baixa Finalitzada' ? 'badge-success' : ($accident->type === 'Amb baixa' ? 'badge-warning' : 'badge-info') }}">
                        {{ $accident->type }}
                    </span>
                </td>
